...
# Optional dependency: pandapower is only required when algorithm == "PandaPower".
# Keep it lazy to allow running Laurent/Tensor PF mode without pandapower installed.
try:
...
class PowerNetEnv(gym.Env):
    ...
    def __init__(self, env_config: dict = env_config) -> None:
        ...
        if self.algorithm == "Laurent":
            ...
            if pp is None:
                raise ImportError(
                    "pandapower 未安装，但当前 env_config['algorithm'] 选择了 'PandaPower'。"
                    "请安装 pandapower 或把 algorithm 改为 'Laurent'。"
                )
