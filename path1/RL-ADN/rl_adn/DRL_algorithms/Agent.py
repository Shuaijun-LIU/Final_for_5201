...
class AgentBase:
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        # Some example scripts call explore_env() before setting `last_state`.
        # Make the agent robust by initializing from env.reset() automatically.
    ...
    def explore_vec_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        if state is None:
            # Best-effort initialization for vector envs: assume env.reset() returns state array.
            ary_state = env.reset()
